...
**使い方は「止める」側と「拮抗の決め手」だけ。**
点が高いことを「直してよい根拠」にはしない。**点が低いものを
出さない**（出口の検品）ためと、**もう直すと決まった場所で
候補どうしを並べる**（`logp_parts`・2026-08-28・うにさんの検討
「言語の特徴を補正に用いて、隣接キーの候補に優先順位を付けたり、
脱字の文字を推測したりする」）ためだけに使う。決め手は
**新しい補正を生まない**——異様判定と門を通った候補の中で
順番を決めるだけなので、項目48-AR（出口の検品で 23.3% 誤判定）
とは場所が違う。
...
def logp_parts(text):
    """
    **合計の対数確率と、数えた3連の数** `(total, n)`（2026-08-28）。

    `score` の中身をそのまま割らずに返す形。候補どうしを比べる
    ときは**合計**で比べる——同じ窓にはめ込んだ候補は共通の3連を
    多く持つので、割ってしまうと差が薄まって順位が揺れる。
    表が無ければ `(0.0, 0)`（＝意見なし）。
    """
    table = _load()
