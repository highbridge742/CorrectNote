...
def _vocab_stamp(store):
    ...
    ★★ **回数と最終使用時刻は、そもそも持っていない**
    （項目48-QG・2026-09-05）。見分けに使うのは
    **語の顔ぶれと、立っているか（solid）**だけ。
    順番に依らないよう XOR で畳む（並べ替えの手間を省くため）。
    """
    try:
        from vocabulary import entry_is_solid as _entry_is_solid
    ...
    for e in entries:
        n += 1
        # **`count` ではなく `solid` を見る**（項目48-QG）。巡1の
        # あいだメモリ上には `count`（solid の写し）も居るが、
        # **見分けが写しのほうを見ていると、巡4 で写しを外した
        # 瞬間に指紋が固まる**（＝古い控えを読み続ける）。
        # 読み方の本体は `vocabulary.entry_is_solid` 1本（48-GN）。
        c = 1 if _entry_is_solid(e) else 0
        total += c
...
def _small_store_stamp(obj, methods):
    ...
    ### ★★ 件数は `len(obj)` で採らない（項目48-QY・2026-09-05）

    `len(DecisionStore)` は**3つの台帳を全部**数える
    （`_rejected` ＋ `_protected` ＋ **`_odd_only`**）。ところが
    `_odd_only`（紫を下げるだけの台帳・48-QU）は**補正の答えを
    1バイトも変えない**ので、ここに入れてはいけない——入れると
    **「紫を1つ下げた」だけで、次の起動でディスクの控えが全部外れる**
    （＝全タブを解析し直す）。

    **見るのは `methods` で渡されたぶんだけ**にする。そうすれば
    「何を見分けに使うか」は**呼び手の1か所で決まる**（48-GN）。
    """
    if obj is None:
        return None
    parts = []
    ...
    return [sum(len(p) for p in parts), _crc(repr(parts))]
